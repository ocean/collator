...
tap.test('did the collator library load', function didLoad(t) {
  t.ok(collator, 'collator library loaded');
  t.end();
});

tap.test('does collator fetch a JSON URL and store it', { timeout: 5000 }, function fetchJSON(t) {
  collator.newsPlease(JSONurl, 'json', false, function getJSON(error, data) {
...
tap.test('does collator force refresh a JSON URL if asked', { timeout: 5000 },
  function refreshJSON(t) {
    collator.newsPlease(JSONurl, 'json', true, function getAgainJSON(error, data) {
      t.error(error, 'there is no error object');
      t.notOk(error, 'error object is falsy');
      t.ok(data, 'data returned is ok');
      t.type(data, 'object', 'data is an object');
...
tap.test('does collator fetch an XML URL and store it', { timeout: 5000 }, function fetchXML(t) {
  collator.newsPlease(XMLurl, 'xml', false, function getXML(error, data) {
    t.error(error, 'there is no error object');
    t.notOk(error, 'error object is falsy');
    t.ok(data, 'data returned is ok');
    t.type(data, 'object', 'data is an object');
...
tap.test('expect an error if we try to load a broken url', { timeout: 5000 },
  function errorWithBroken(t) {
    collator.newsPlease('http://this-does-not-exist-really.in', 'json', false, function getBroken(error, data) {
